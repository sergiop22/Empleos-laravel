<template>
	<div>
		<ul class="flex flex-wrap justify-center">
			<li  class="border border-gray-500 px-10 py-3 mb-3 rounded mr-4" v-for="( skill, i ) in this.skills" v-bind:key="i" @click="activar($event)">{{skill}}</li>
		</ul>
		...
		methods: {
			activar(e) {
				if( e.target.classList.contains('bg-teal-400')){
					...
					//agregar al set las habilidades
					this.habilidades.add(e.target.textContent);
				}

				//agregar habilidades al input
